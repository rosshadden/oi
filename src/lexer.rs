...
pub enum Token {
	...
	#[regex(r"0[xX][0-9a-fA-F][0-9a-fA-F_]*", |lex| i32::from_str_radix(&lex.slice()[2..].replace('_', ""), 16).ok())]
	#[regex(r"0[bB][01][01_]*", |lex| i32::from_str_radix(&lex.slice()[2..].replace('_', ""), 2).ok())]
	#[regex(r"0[oO][0-7][0-7_]*", |lex| i32::from_str_radix(&lex.slice()[2..].replace('_', ""), 8).ok())]
	Int(i32),
